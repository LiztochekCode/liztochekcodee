...
export const registerCommands = (ctx: ExtensionContext) => {
    ...
    const enableCommand = commands.registerCommand("liztochekcode.enable", async () => {
        await disable(false);
        await enable(false);
    ...
    const disableCommand = commands.registerCommand("liztochekcode.disable", async () => {
        await disable(false);
    ...
    const enableWorkspaceCommand = commands.registerCommand("liztochekcode.workspace.enable", async () => {
        await disable();
        await enable();
    ...
    const disableWorkspaceCommand = commands.registerCommand("liztochekcode.workspace.disable", async () => {
        await disable();
    ...
    const reconnectCommand = commands.registerCommand("liztochekcode.reconnect", async () => {
        ...
        await controller
            ...
            .catch(() => {
                ...
                controller.statusBarIcon.command = "liztochekcode.reconnect";
                controller.statusBarIcon.show();
            });
    });

    const disconnectCommand = commands.registerCommand("liztochekcode.disconnect", async () => {
